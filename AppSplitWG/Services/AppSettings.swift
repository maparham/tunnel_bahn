...
final class AppSettings: ObservableObject {
    ...
    /// One-time initialization guard for Per-App mode defaults (select all apps).
    ...
    /// Merge the host app into `NEAppRule` so AppSplitWG-initiated probes use the tunnel under per-app VPN.
    ...
    init() {
        ...
        if let raw = AppGroupStore.defaults.string(forKey: Keys.routingMode),
           let decoded = RoutingMode(rawValue: raw) {
            routingMode = decoded
        } else {
