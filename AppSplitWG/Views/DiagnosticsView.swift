...
struct DiagnosticsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            GroupBox("Per-App VPN Verification") {
                VStack(alignment: .leading, spacing: 8) {
                    Text("To verify per-app split tunneling is working:")
                        .font(.headline)
                    
                    Text("""
                    1. Set "Unmatched apps" to "Bypass VPN" in Settings (required for per-app split)
                    2. In Apps tab, toggle ONLY Firefox to "Route via VPN"
                    3. Connect to VPN
                    4. Check Manager Diagnostics shows:
                       - Routing Method: sourceApplication (per-app VPN)
    ...
    private func runVerification() {
        ...
        Task {
            ...
            let hasCorrectRoutingMethod = diag.contains("sourceApplication (per-app VPN)")
            ...
            let hasNoAppRules = diag.contains("No app rules configured (full tunnel will route ALL traffic)") ||
                diag.contains("No app rules configured (per-app VPN will route NO apps)")
            ...
            if hasCorrectRoutingMethod && hasOnDemandEnabled && hasAppRules {
                results += "✅ Configuration looks correct for per-app VPN!\n\n"
                results += "Next steps:\n"
                ...
                results += "❌ Configuration has issues. Per-app VPN may not work.\n\n"
