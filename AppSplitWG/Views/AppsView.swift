...
struct AppsView: View {
    ...
    private var canEnablePerAppMode: Bool {
    ...
    private var routingModeSelection: Binding<AppSettings.RoutingMode> {
        Binding(
            ...
            set: { newValue in
                if newValue == .perApp, !canEnablePerAppMode { return }
                appState.settings.routingMode = newValue
            }
        )
    }

    private var perAppRadioLabel: String {
        if canEnablePerAppMode {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if !AppConstants.isPerAppSplitTunnelEnabled {
                Picker("Mode", selection: routingModeSelection) {
                    ...
                    Text(perAppRadioLabel).tag(AppSettings.RoutingMode.perApp)
                        .disabled(!canEnablePerAppMode)
                }
            ...
            if !AppConstants.isPerAppSplitTunnelEnabled {
                Text(
                    "Per-app routing is off in this build: every connect is a full tunnel (all apps). The list below is ignored until split tunnel is re-enabled in code."
                )
                .font(.caption)
            ...
            if AppConstants.isPerAppSplitTunnelEnabled {
                HStack(alignment: .top, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        routingModeRadioRow(
                            mode: .perApp,
                            label: perAppRadioLabel,
                            rowDisabled: !canEnablePerAppMode
                        )
        ...
        .task {
            ...
            // Handle invalid state: per-app mode with zero selected apps
    ...
    private func autoSwitchToFullTunnelIfNoSelection() {
        ...
        guard appState.settings.routingMode == .perApp else { return }
