...
struct StatusView: View {
            ...
            GroupBox("Session") {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if appState.vpnManager.stats.perAppSplitTunnelActive {
                        infoRow("Per-App Split Tunnel", "Active")
                    }
                }
        ...
        GroupBox {
            VStack(alignment: .leading, spacing: 8) {
                ...
                if displayedPerAppStats.isEmpty {
                    Text("Waiting for per-app traffic…")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
    ...
    private var tunnelModeLabel: String {
        ...
        let planned: String = {
            ...
            case .perApp:
                // Option A: Per-App mode with zero selected apps behaves like Full Tunnel.
                return selectedRoutedApps > 0 ? "Per-App (selected apps only)" : "Full Tunnel (all traffic)"
            }
        }()
        let active = appState.vpnManager.stats.perAppSplitTunnelActive ? "Per-App (selected apps only)" : "Full Tunnel (all traffic)"
